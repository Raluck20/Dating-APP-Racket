...
(define (get-unstable-couples engagements mpref wpref)
  (let ((unstable-couples '()))
    (map (lambda (pair) (let* ((a (car pair)) (d (cdr pair)) (topman (get-pref-list wpref a)) (topwoman (get-pref-list mpref d)))
           (cond ((better-match-exists? a d topman mpref (map (lambda (x) (let* ((xd (cdr x)) (xa (car x))) (cons xd xa))) engagements))
