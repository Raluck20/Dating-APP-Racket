...
(define (get-men mpref)
  (map (append (lambda (x) (car x))) mpref))
...
(define (get-women wpref)
   (foldr (lambda (x y) (cons (car x) y)) '() wpref))
...
(define (get-partner engagements person)
  ((lambda (partner-pair)
     (if partner-pair
         (if (equal? (car partner-pair) person)
             (cdr partner-pair)
             (car partner-pair))
         #f))
   (find-first
...
(define (update-engagements engagements p1 p2)
  (change-first (lambda (p) (equal? p1 (car p))) 
                engagements 
                (cons p1 p2)))
