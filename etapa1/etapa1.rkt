...
(define (get-women wpref)
  (get-women-list wpref '() ))

(define (get-women-list wpref L)
  (if(null? wpref)
     (reverse L)
     (get-women-list (cdr wpref)(cons (car (car wpref)) L)))
  )
...
(define (get-pref-list pref person)
  (get-pref-ajutor pref person '() ))

(define (get-pref-ajutor pref person L)
  (if(null? pref)
     L
     (if(equal? (car(car pref)) person)
     (append (cdr(car pref)) L)
     (get-pref-ajutor(cdr pref) person L)))    
  )
...
(define (preferable? pref-list x y)
  (or (and (equal? (car pref-list) x) (not (equal? (car pref-list) y)))
      (and (not (equal? (car pref-list) x)) (not (equal? (car pref-list) y))
...
(define (get-partner engagements person)
  (cond
    ((null? engagements) #f)
    ((equal? (car(car engagements)) person)(cdar engagements))
